...
Value GetNetworkHashPS(int lookup, int height) {
    ...
    // Mozzshare difficulty changes every block on downward diff an
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Mozzshare is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Mozzshare is downloading blocks...");
