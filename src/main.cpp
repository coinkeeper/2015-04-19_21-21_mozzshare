...
int64 nChainStartTime = 1407218400;
uint256 hashGenesisBlock("0x0000f31e7eed3db329dddb977cdad26e68f6da4b4f02834ecc49799a0b98d5e0");
...
uint16_t GetCurrentBlockReward(CBlockIndex *pindexPrev)
{
   // if (pindexPrev->getSupply() >= nMaxSupply)
	...
	else if(nHeight <= 1440)// 1date: d1
	{
		nBlockReward = 4375 ;

	}
	else if(nHeight <= 11520)// 7dates: d2-d8
	{
		nBlockReward = 1041 ;
	}
	else if(nHeight <= 54720)// 30dates: d9-d38
	{
		nBlockReward = 388 ;
	}
...
const int64 nTargetTimespan = 1 * 60; // 1 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Mozzshare Temporal Retargeting - exits "blackhole" in ~3hrs
    if (pindexLast->nHeight > nLifeWindow && count < nLifeWindow/4) {
        int min = nLifeWindow/4 - count;
        printf("Retarget: **** Mozzshare Temporal Retargeting **** %d/%d : factor = %d\n",
...
void ThreadScriptCheck() {
    RenameThread("mozzshare-scriptch");
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x0000d08a1f00c71f66e4da6c33f68c435afcc86f2c3a1915161fc51a24d03d8d");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "NY Times 05/08/2014 NYT Offering New Digital Subscription Plans";
        CTransaction txNew;

        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = nPremineMoney;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("03a7de6ada4ee16b9fd744e6a3c9db786e4f3aec1508bf8f0d0a200b9c675e26cc") << OP_CHECKSIG;
        ...
        block.setVersion(1);
        block.setSupply(nPremineMoney/COIN);
        block.nTime    = nChainStartTime + 15;
        block.nBits    =bnProofOfWorkLimit.GetCompact();  // HEFTY1 is very expensive, so starts with lower difficulty
        block.nNonce   = 94958;
  ...
  ///////////////////////////////////////////////////////////////////////////
        ...
        while (hashGenesisNew > bnProofOfWorkLimit.getuint256()){
			...
			//if(block.nNonce % 100 == 0)
			   // printf("nonce=%u hashGenesisNew=%s\n",block.nNonce, hashGenesisNew.ToString().c_str());
        }
		...
		block.print();
		assert(block.getSupply() == nPremineMoney/COIN);
        assert(block.hashMerkleRoot == uint256("0xb8875ec15d54274ea740b46698437f1cc7fd361bed53b9d2cfaeabe4c95e3511"));
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    else if (strCommand == "block" && !fImporting && !fReindex) // Ignore blocks received while importing
    {
        ...
        if (state.IsInvalid(nDoS))
            ...
            //AdjustBlockRewardVoteLimit(pindexBest); //TODO
...
CBlockTemplate* CreateNewBlock(CReserveKey& reservekey)
{
    ...
    int64 nFees = 0;
    {
		...
		//TODO
		// printf("pblock->vtx[0].vout[0].nValue %"PRI64u"\n", pblock->vtx[0].vout[0].nValue);
...
void static MozzshareMiner(CWallet *pwallet)
{
    printf("MozzshareMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("mozzshare-miner");
    ...
    try { loop {
        ...
        printf("Running MozzshareMiner with %"PRIszu" transactions in block (%u bytes)\n",
               pblock->vtx.size(),
    ...
    {
        printf("MozzshareMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nGenerateThreads; i++)
        minerThreads->create_thread(boost::bind(&MozzshareMiner, pwallet));
}
